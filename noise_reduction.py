...
# Params
RESCALE_WIDTH = int(2**9)   # 2**8=256. 2**9=512
filts_test = 32


# -----------------------------------------------------------------------------------------------------------
# Parámetros y Ajustes del modelo

# Frecuencia de entrada de los audios
SAMPLING_FREQ = 48000
...
TOP_DB = 80.0
CHANNELS = 1
# Redimensionamiento del espectrograma
...
NORMALIZE = True # False para desactivar normalización
# Ajustes de Modelo
PERCENT = 0.06
TRAINING_PERCENT = 0.7
VALIDATION_PERCENT = 0.2
TEST_PERCENT = 1 - (TRAINING_PERCENT + VALIDATION_PERCENT)
BATCHES = 16
PREFETCH_BATCHES = 1 # tf.data.experimental.AUTOTUNE
N_FILTERS = 2**5 # Número de Filtros. 2**4=16. 2**5=32. 2**6=64
KERNEL_SIZE = 3 # Tamaño del Kernel
ACTIVATION_LAYER = 'relu' # Capa de Activación: tf.keras.layers.ReLU() tf.keras.layers.LeakyReLU(alpha=0.01)
DROPOUT = 0.4
BATCH_NORMALIZATION = False
KERNEL_INIT = 'HeNormal' # Inicialización de Kernel: tf.keras.initializers.HeNormal() tf.keras.initializers.GlorotNormal()
OPTIMIZER = 'adam' # Optimizador. adadelta, adam 
LOSS = 'mae' # Función de perdida: Huber, mse, mae
OUTPUT_ACTIVATION_LAYER = None # None, sigmoid
N_FILTERS_OUT = CHANNELS # Número de filtros en convolución de salida.
# Ajustes de entrenamiento
EPOCHS = 40 # Número de epocas
VERBOSE_MODEL = True # Debugging
# Ajustes de Early Stopping
MIN_DELTA = 0.00001 # Cambio minimo considerado como mejora
PATIENCE = 25 # Número de épocas sin mejora tras las cuales se detendrá el entrenamiento.
SHUFFLE_MODEL = True # Mantener o alterar la secuencia de los datos
CACHE = False


def ajuste_resultados_predecibles():
    random.seed(7)
    np.random.seed(7)
...
#     stft_mag_lineal = lb.db_to_amplitude(stft_mag_db, ref=1.0)
...
#     istft = tf.signal.inverse_stft(audio_reverse_stft,
#                                    fft_length=n_fft,
#                                    frame_step=hop_length,
#                                    frame_length=win_length,
#                                    window_fn=tf.signal.inverse_stft_window_fn(frame_step=hop_length,forward_window_fn=window),
#                                   )
    ...
    if verbose == True:
        ...
        print(f'Duración del audio: {(istft.shape[0]/fs)*1000:.2f} ms.')
...
if __name__ == '__main__':
    print("Cargando modelo Noise reduction")
    ajuste_resultados_predecibles()
